...
with c1:
    df_estado = get_top_counts("Estado Resumen", 10)
    if not df_estado.empty:
        fig = px.pie(df_estado, names="label", values="value", title="Distribución por estado resumen")
        st.plotly_chart(fig, use_container_width=True)

with c2:
    df_proc = get_top_counts("Estado del Procedimiento", 10)
    if not df_proc.empty:
        fig = px.pie(df_proc, names="label", values="value", title="Distribución por estado del procedimiento")
        st.plotly_chart(fig, use_container_width=True)

c3, c4 = st.columns(2)

with c3:
    df_val_tipo = get_top_values("Tipo de Contrato", "Valor Total Adjudicacion", 10)
...
with c4:
    df_val_modalidad = get_top_values("Modalidad de Contratacion", "Valor Total Adjudicacion", 10)
...
df_city = get_top_counts("Ciudad Entidad", 15)
st.dataframe(df_city, use_container_width=True, hide_index=True)

st.subheader("Top categorías por valor adjudicado")
df_cat_val = get_top_values("Codigo Principal de Categoria", "Valor Total Adjudicacion", 15)
